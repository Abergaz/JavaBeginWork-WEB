...
public class FrameMain extends javax.swing.JFrame implements WSResultListener {
    ...
    private FlightRSClient searchClient; // ссылка на клиент веб сервиса
...
//        ProxySelector.setDefault(new CustomProxySelector());
        searchClient = new FlightRSClient();

        // вызов веб сервиса для получения списка городов
        // CityList.class - тип возращаемого значения
        CityList cityList = (CityList) searchClient.getAllCities(CityList.class);

        // конвертация из City в ExtCity
        ArrayList<ExtCity> list = new ArrayList<>();
        try {
    ...
    private void btnBuyActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnBuyActionPerformed
        ...
        if (tableFlights.getRowCount() == 0) { // если нет найденных рейсов - ничего не делаем
            return;
        }

        int selectedRow = 0; // по-умолчанию выбираем первую строку

        selectedRow = tableFlights.getSelectedRow();

        Flight flight = flightList.get(tableFlights.getSelectedRow());

        DialogBuy dialogBuy = new DialogBuy(this, true, searchClient);
        dialogBuy.setFlight(flight);
        dialogBuy.setVisible(true);
        
        searchFlights(); // после покупки - обновляем список рейсов (чтобы получить актуальную информацию по местам и пр.)
    ...
    private void btnCheckTicketActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnCheckTicketActionPerformed
        DialogCheck dialogCheck = new DialogCheck(this, true, searchClient);
    ...
    private void searchFlights() {
        ...
        // вызов веб сервиса
        // FlightList.class - тип возращаемого значения (обертка)
        FlightList flightListResult = (FlightList) searchClient.searchFlight(null, FlightList.class, dateFrom.toString(), String.valueOf(cityFrom.getId()), String.valueOf(cityTo.getId()).toString());

        updateFlights(flightListResult.getFlightList()); // заполняем таблицу
    }
